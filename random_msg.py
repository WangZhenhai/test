...
a1 = ['赵', '钱', '孙', '李', '周', '吴', '郑', '王', '公良', '拓跋', '西门', '南宫', '第五', '公仪', '太史', '贺兰', '独孤', '南郭', '北宫']
...
def random_mobile():
	# for i in range (count):
	mobile = r.choice (m1) + r.choice (m2) + r.choice (m2) + r.choice (m2) + r.choice (m2) + r.choice (
			m2) + r.choice (m2) + r.choice (m2) + r.choice (m2)
	return mobile
...
def random_idcard():
	# for i in range (count):
	idcard = r.choice (id1) + y + r.choice (year) + r.choice (id2) + r.choice (id3) + r.choice (id4) + r.choice (
			id5) + r.choice (id6) + r.choice (id7)
	return idcard


def random_bankNum():
	# for i in range (count):
	bankNum = bank + r.choice (bank_num) + r.choice (bank_num) + r.choice (bank_num) + r.choice (
			bank_num) + r.choice (bank_num) + r.choice (bank_num) + r.choice (bank_num) + r.choice (
			bank_num) + r.choice (bank_num) + r.choice (bank_num)
	return bankNum
...
if __name__ == "__main__":
	for i in range (count):
		with open ("aaa.txt", "a+") as f:
			f.write (
				random_name () + "," + random_mobile () + "," + random_idcard () + "," + random_bankNum () + "," + "招商银行|CMB\n")
			print (
				random_name () + "," + random_mobile () + "," + random_idcard () + "," + random_bankNum () + "," + "招商银行|CMB")
			f.close ()
